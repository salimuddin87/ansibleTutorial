...
# wait for analyticjsonexport,  until pod is up and running
...
#
# Usage:
#        waitForExportPod.sh namespace


if [ "$#" -ne 1 -o -z $1 ]; then
   echo "ERROR: namespace not passed. "
   echo "Usage: waitForExportPod.sh namespace "
   echo "Exiting..."
   exit 1
fi
...
while true
   do
      ...
      if [[ $PodIp =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$ && $Status == Succeeded || $Status == Running ]]; then
         ...
         echo “Still waiting for the analyticjsonexport pods to be running ....”
      fi
